...
// Okres okresowej weryfikacji (aktywnego utrzymania) wyjsc A460 [ms]
static const uint16_t MAINTAIN_OUTPUTS_PERIOD_MS = 1000;
...
static const uint8_t  SERIOUS_ERROR_THRESHOLD = 3;
...
// Wersja firmware
static const char *const FW_VERSION = "1.8";
...
// Zadania okresowe: skan E160 (krok 1 ms), debouncing, gesty,
// oraz okresowe utrzymanie/weryfikacja wyjsc A460.
void processPeriodicTasks(void)
{
    ...
    // Okresowe aktywne utrzymanie i weryfikacja wyjsc (tylko tryb normalny)
    if (runMode == MODE_NORMAL)
    {
        if ((uint16_t)(now - lastMaintainMs) >= MAINTAIN_OUTPUTS_PERIOD_MS)
        {
            lastMaintainMs = now;
            maintainOutputs();
        }
